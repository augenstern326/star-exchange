...
export default function CreateTask() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.deadline_at) {
            const selectedDate = new Date(formData.deadline_at);
            const today = new Date();
            today.setHours(0, 0, 0, 0);

            if (selectedDate < today) {
                message.error('截止日期不能早于今天');
                return;
            }
        }
        ...
        try {
            const taskData = {
                ...
                deadline_at: formData.deadline_at || null, // 如果为空字符串则传null
            };
            ...
            <div className="max-w-2xl mx-auto px-4 py-8">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <Card className="p-6 bg-white space-y-4">
                        ...
                        {/* Deadline Section - 简化的截止日期部分 */}
                        {taskType==='approval' &&(<div className="pt-2">
                            <Label htmlFor="deadline_at" className="block mb-2">
                                截止日期
                            </Label>
                            <div className="relative">
                                <Input
                                    ...
                                    className="text-lg py-6 pl-4 pr-10"
                                    min={format(new Date(), 'yyyy-MM-dd')} // 限制不能选择过去的日期
                                />
                            ...
                            {formData.deadline_at && (
                                <div className="text-sm text-pink-600 font-medium mt-1">
                                    已选择：{format(new Date(formData.deadline_at), 'yyyy年MM月dd日')}
                                </div>
                            )}
                        </div>)}
